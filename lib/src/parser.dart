...
class Parser {
  ...
  Future<ParsedData> parseProject() async {
    ...
    AnalysisContextCollection collection = new AnalysisContextCollection(includedPaths: [environment.config.input]);
    ...
    for (var absolutePath in environment.package.absolutePaths) {
      ...
      AnalysisContext context = collection.contextFor(absolutePath); // TODO or relative?
      (await context.currentSession.getResolvedUnit(absolutePath)).unit.accept(new ASTVisitor(environment, absolutePath, parsedData)); // TODO or relative?
    }
    return parsedData;
  }
}
